...
class LargeVocabUnetGenerator(nn.Module):

    def __init__(self,in_dim,out_dim,num_filter,num_character_sets,norm_type='gn'):
        super(LargeVocabUnetGenerator,self).__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.num_filter = num_filter
        act_fn = nn.LeakyReLU(0.2, inplace=True)
        self.num_character_sets = num_character_sets
        ...
        for k in range(num_character_sets):
            out_module_lst.append(nn.Sequential(
        ...
        self.out = nn.ModuleList(out_module_lst)
    ...
    def forward(self,input,character_set):
        assert character_set >= 0 and character_set < self.num_character_sets
        ...
        out = self.out[character_set](up_4)
...
if __name__ == "__main__":
    ...
    model = LargeVocabUnetGenerator(3, 100, 64,4)
    ...
    #print (model(x, 0)[0]**2).sum()
    #print (model(x, 3)[0]**2).sum()
